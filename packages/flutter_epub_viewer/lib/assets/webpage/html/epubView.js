...
function getSelectionScrollContainer(contents) {
  try {
    if (!contents || !contents.document) return null;
    return contents.document.scrollingElement || contents.document.documentElement || contents.document.body;
  } catch (e) {
    return null;
  }
}

function getOuterViewportContainer() {
  return document.querySelector('.epub-container') || document.getElementById('viewer');
}

function releaseSelectionViewportLock() {
  try {
    var contents = selectionViewportLock.contents;
    if (contents && contents.document) {
      var doc = contents.document;
      var scroller = getSelectionScrollContainer(contents);
      if (scroller) {
        if (selectionViewportLock._innerScrollHandler) {
          scroller.removeEventListener('scroll', selectionViewportLock._innerScrollHandler, true);
        }
        if (selectionViewportLock._touchMoveHandler) {
          doc.removeEventListener('touchmove', selectionViewportLock._touchMoveHandler, true);
        }
        if (selectionViewportLock._wheelHandler) {
          doc.removeEventListener('wheel', selectionViewportLock._wheelHandler, true);
        }
    ...
    var container = selectionViewportLock.container;
    if (container) {
      if (selectionViewportLock._containerScrollHandler) {
        container.removeEventListener('scroll', selectionViewportLock._containerScrollHandler, true);
      }
      container.style.touchAction = '';
    }

    if (selectionViewportLock._windowScrollHandler) {
      window.removeEventListener('scroll', selectionViewportLock._windowScrollHandler, true);
    }
  ...
  selectionViewportLock.active = false;
  selectionViewportLock.contents = null;
  selectionViewportLock.iframe = null;
  selectionViewportLock.container = null;
  selectionViewportLock._innerScrollHandler = null;
  selectionViewportLock._containerScrollHandler = null;
  selectionViewportLock._windowScrollHandler = null;
  selectionViewportLock._touchMoveHandler = null;
  selectionViewportLock._wheelHandler = null;
  if (selectionViewportLock.rafId) {
    cancelAnimationFrame(selectionViewportLock.rafId);
    selectionViewportLock.rafId = null;
  }
}

function lockViewportForSelection(contents) {
  try {
    var iframe = null;
    try {
      iframe = contents && contents.document && contents.document.defaultView
    ...
    selectionViewportLock.active = true;
    selectionViewportLock.contents = contents || null;
    selectionViewportLock.iframe = iframe;
    selectionViewportLock.container = getOuterViewportContainer();

    var innerScroller = getSelectionScrollContainer(contents);
    selectionViewportLock.innerScrollLeft = innerScroller ? innerScroller.scrollLeft || 0 : 0;
    selectionViewportLock.innerScrollTop = innerScroller ? innerScroller.scrollTop || 0 : 0;
    selectionViewportLock.containerScrollLeft =
      selectionViewportLock.container ? selectionViewportLock.container.scrollLeft || 0 : 0;
    selectionViewportLock.containerScrollTop =
      selectionViewportLock.container ? selectionViewportLock.container.scrollTop || 0 : 0;

    if (innerScroller) {
      innerScroller.style.touchAction = 'none';
    }
    if (contents && contents.document) {
      ...
      if (contents.document.body) {
        contents.document.body.style.touchAction = 'none';
      }
    }
    if (selectionViewportLock.container) {
      selectionViewportLock.container.style.touchAction = 'none';
    }
  } catch (e) {
    console.error('Error locking selection viewport:', e);
  }
}

// Apply CSS to a single contents iframe
function applyCustomCssToContents(contents) {
  try {
    if (!contents || !contents.document) return;
    var doc = contents.document;
    var existing = doc.getElementById("custom-epub-css");
    if (!existing) {
      existing = doc.createElement("style");
      existing.id = "custom-epub-css";
      doc.head.appendChild(existing);
    }
    existing.innerHTML = customCssText || "";
  } catch (e) {
    console.error("Error applying custom CSS:", e);
  }
}

// Public hook callable from Dart
function setCustomCss(cssText) {
  customCssText = cssText || "";
  try {
    if (rendition && rendition.getContents) {
      rendition.getContents().forEach(function (contents) {
        applyCustomCssToContents(contents);
      });
    }
  } catch (e) {
    console.error("Error applying CSS to contents:", e);
  }
}

// When new iframe content loads, apply CSS automatically
function registerCustomCssHook() {
...
function loadBook(data, cfi, initialXPath, manager, flow, spread, snap, allowScriptedContent, direction, useCustomSwipe, backgroundColor, foregroundColor, fontSize, clearSelectionOnNav, selectAnnotationRangeParam) {
  ...
  // ✅ make sure CSS hooks are active
  registerCustomCssHook();
  ...
  // Clamp the selection range to the visible viewport so it can't extend
  // into the next/previous page (CSS column) which would cause cfiFromRange errors
  function clampSelectionToViewport(contents) {
    try {
      ...
      // Check if any rect extends beyond the visible viewport
      var lastRect = rects[rects.length - 1];
      var firstRect = rects[0];
      var needsClamp = (lastRect.right > viewportWidth + 5) ||
        (firstRect.left < -5);

      if (!needsClamp) return;

      // Selection extends beyond viewport — clamp to the last visible position
      // Use caretRangeFromPoint to find the character at the viewport edge
      if (typeof doc.caretRangeFromPoint === 'function') {
        // Clamp right edge: find the last character inside the viewport
        if (lastRect.right > viewportWidth + 5) {
          var clampX = viewportWidth - 8;
          // Try multiple Y positions to find a valid caret position
          var clampY = lastRect.top + (lastRect.height / 2);
          var caretRange = doc.caretRangeFromPoint(clampX, clampY);
          if (!caretRange) {
            // Try middle of viewport height
            caretRange = doc.caretRangeFromPoint(clampX, viewportHeight / 2);
          }
          if (caretRange) {
            try {
              ...
              // If setEnd fails (e.g. wrong document order), just ignore
            }
          }
        }
        // Clamp left edge
        if (firstRect.left < -5) {
          var clampX2 = 8;
          var clampY2 = firstRect.top + (firstRect.height / 2);
          var caretRange2 = doc.caretRangeFromPoint(clampX2, clampY2);
          if (caretRange2) {
            try {
              range.setStart(caretRange2.startContainer, caretRange2.startOffset);
              selection.removeAllRanges();
              selection.addRange(range);
            } catch (e) {
              // Ignore
      ...
      // Ignore errors — clamping is best-effort
    }
  }

  // Handle selection clearing and changes
  rendition.hooks.content.register(function (contents) {
    // Suppress native context menu (copy/paste/share popup) inside epub content
    contents.window.document.addEventListener('contextmenu', function (e) {
      e.preventDefault();
      e.stopPropagation();
      return false;
    }, { capture: true });

    // Also suppress long-press callout on iOS
    try {
      var suppressStyle = contents.document.createElement('style');
      suppressStyle.id = 'suppress-native-menu';
      suppressStyle.textContent = '* { -webkit-touch-callout: none !important; }';
      contents.document.head.appendChild(suppressStyle);
    } catch (e) {
      // Ignore
    }
    ...
    contents.window.document.addEventListener('selectionchange', function () {
      ...
      if (!hasNonCollapsedRange && !selectedText) {
        ...
        lockViewportForSelection(actualContents);

        // Clamp the selection inside the currently active visible contents,
        // not the original event source, which can be stale after pagination.
        clampSelectionToViewport(actualContents);
        ...
        selectionTimeout = setTimeout(function () {
          ...
          lockViewportForSelection(finalContents);

          // Final clamp before processing — ensure selection stays inside
          // the visible viewport of the actual active contents.
          clampSelectionToViewport(finalContents);
  ...
  rendition.on("selected", function (cfiRange, contents) {
    lastCfiRange = cfiRange;
    lockViewportForSelection(contents);
  ...
  rendition.on("relocated", function (location) {
    ...
    // This is the final safeguard: while a selection exists, keep the reader
    // on the last stable location and do not allow paginated relocation.
    if (hasActiveSelection()) {
      console.log('Blocked relocated event - selection active, attempting to revert');
      lockViewportForSelection(selectionViewportLock.contents || null);
      ...
      if (rendition.location && rendition.location.start) {
        try {
          ...
          // Only revert if the current location is different from lastValidLocation
          // and we actually have a stable location to return to.
          if (window.lastValidLocation &&
            ...
            setTimeout(function () {
              // Double-check that selection is still active
              if (!hasActiveSelection()) {
                return; // Don't revert if selection was already cleared
              }
  ...
  rendition.hooks.content.register((contents) => {
    ...
    doc.addEventListener('touchmove', function (e) {
      if (hasActiveSelection()) {
        ...
        // While selection exists, freeze the reading viewport completely.
        e.preventDefault();
        e.stopPropagation();
...
function blockGesturesWhenSelected(block) {
  try {
    ...
    if (block) {
      // Block ALL panning/swiping when selection exists
      // Use 'none' to prevent any touch scrolling that could shift the viewport
      // and reveal adjacent pages in the continuous manager
      if (!existingStyle) {
      ...
      function applyTouchActionToIframes() {
        try {
          ...
          allIframes.forEach(function (iframe) {
            ...
            iframe.style.setProperty('-ms-touch-action', 'none', 'important');
            iframe.style.setProperty('touch-action', 'none', 'important');
          });
        } catch (e) {
...
function checkSelectionAndReapplyBlocking() {
  try {
    ...
    if (hasSelection) {
      lockViewportForSelection(selectionViewportLock.contents);
      var styleId = 'epub-selection-block-style';
      var existingStyle = document.getElementById(styleId);

      if (!existingStyle) {
        // Re-apply blocking if it was removed - use 'none' to block all touch movement
        var style = document.createElement('style');
        style.id = styleId;
        style.textContent = `
          body, html, #viewer {
...
function detectSwipe(el, func) {
  ...
  ele.addEventListener('touchmove', function (e) {
    // If selection exists, block ALL movement to prevent viewport shift
    if (hasActiveSelection()) {
      var t = e.touches[0];
      ...
      // Block any significant movement (horizontal OR vertical)
      // to prevent the continuous manager from shifting the viewport
